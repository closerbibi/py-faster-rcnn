function grid = slicedto2D(points3D,normals,count,xmin,xmax,ymin,ymax,clss)
...
for rot = 1:7
    ...
    for kr = 1:size(xmax,2)
        tmp1 = R*[xmin(kr);ymin(kr)];
        tmp2 = R*[xmax(kr);ymax(kr)];
    ...
    [gridindex_rot,xmin_rot(:,:,rot),xmax_rot(:,:,rot),ymin_rot(:,:,rot),ymax_rot(:,:,rot)] = turn2Dpoints2grid(points2D_rot(:,:,rot),xmin_rot(:,:,rot),xmax_rot(:,:,rot),ymin_rot(:,:,rot),ymax_rot(:,:,rot));
    width_rot = max(gridindex_rot(1,:));
    height_rot = max(gridindex_rot(2,:));
    %% trim the box out of range
    [xmin_rot(:,:,rot),xmax_rot(:,:,rot),ymin_rot(:,:,rot),ymax_rot(:,:,rot)] = trim_boxoutofscene(xmin_rot(:,:,rot),xmax_rot(:,:,rot),ymin_rot(:,:,rot),ymax_rot(:,:,rot),width_rot,height_rot);
...
%         save(sprintf('./picture_forPooling/picture_%06d_%03d_%02d.mat',count,k,rot),'grid');
        d = d-step_size;
    end
...
%     save(sprintf('./picture_forPooling/picture_%06d_%02d.mat',count,rot),'grid_whole_img');
    
    %% save ground truth annotation
% % %     annofile = sprintf('~/bin/faster-rcnn/data/DIRE/Annotations/picture_%06d_%02d.txt',count,rot);
