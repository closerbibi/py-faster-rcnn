...
path_source='~/3D/understanding/rankpooling/feature/target_class_only/rankpooling/picture_forPooling';
%path='~/bin/faster-rcnn/data/v_BlowDryHair_g12_c04_mp4';
for v=1:nScenes
  % pool among frames
  indv = 1:25;%((v-1)*25+1):((v-1)*25+25);%find(ids==v);
  checksize=load(sprintf('%s/picture_%06d.mat',path_source,(v-1)*25+1),'grid');
  imgsize = size(checksize.grid);
  X=zeros([imgsize,25]);
  for count = 1:25
	  tmp=load(sprintf('%s/picture_%06d.mat',path_source,(v-1)*25+count),'grid');
	  X(:,:,:,count)=tmp.grid;
  end
  if isempty(indv)
